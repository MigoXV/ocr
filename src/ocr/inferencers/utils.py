...
def _is_image_part(part: Any) -> bool:
    if part_get(part, "type") == "image_url":
        return True
    image_url = part_get(part, "image_url")
    ...
    return bool(getattr(image_url, "url", None))
...
def extract_message_text(message: ChatCompletionMessageParam) -> str:
    ...
    if isinstance(content, Sequence) and not isinstance(content, (str, bytes)):
        parts: list[str] = []
        for part in content:
            if part_get(part, "type") == "text":
                text = part_get(part, "text")
...
def part_get(part: Any, key: str) -> Any:
